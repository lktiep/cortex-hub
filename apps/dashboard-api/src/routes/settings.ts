...
settingsRouter.put('/hub-config', async (c) => {
  ...
  const update = db.prepare(
    "UPDATE hub_config SET value = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now') WHERE key = ?"
  )
...
settingsRouter.put('/embedding-provider', async (c) => {
  ...
  db.prepare(
    "INSERT INTO hub_config (key, value, updated_at) VALUES ('embedding_provider', ?, strftime('%Y-%m-%dT%H:%M:%SZ', 'now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at"
  ).run(body.provider)
  if (body.model) {
    db.prepare(
      "INSERT INTO hub_config (key, value, updated_at) VALUES ('local_embedding_model', ?, strftime('%Y-%m-%dT%H:%M:%SZ', 'now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at"
    ).run(body.model)
  }
...
settingsRouter.put('/notifications', async (c) => {
  ...
  const upsert = db.prepare(
    "INSERT INTO notification_preferences (key, enabled, updated_at) VALUES (?, ?, strftime('%Y-%m-%dT%H:%M:%SZ', 'now')) ON CONFLICT(key) DO UPDATE SET enabled = excluded.enabled, updated_at = excluded.updated_at"
  )
